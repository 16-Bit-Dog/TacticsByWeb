...
namespace UnityTemplateProjects
{
    public class SimpleCameraController : MonoBehaviour
    {
        ...
        public bool mouseCamSlide = false;

        public float DrawBoundX = 0;

        public float DrawBoundY = 0;

    void DragCamMouse()
        {

            if(mouseCamSlide == true)
            {
                Vector3 VecMovVal;

                Vector3 mPos = Input.mousePosition;
                if (mPos.x > (Screen.width-30) )
                {
                    VecMovVal = new Vector3(0.1f,0.0f,0.0f);
                    m_TargetCameraState.Translate(VecMovVal);
                }

                else if(mPos.x < (30))
                {
                    VecMovVal = new Vector3(-0.1f, 0.0f, 0.0f);
                    m_TargetCameraState.Translate(VecMovVal);//move cam left
                }

                if (mPos.y > (Screen.height - 30))
                {
                    VecMovVal = new Vector3(0.0f, 0.1f, 0.0f);
                    m_TargetCameraState.Translate(VecMovVal);//move cam down
                }

                else if (mPos.y < (30))
                {
                    VecMovVal = new Vector3(0.0f, -0.1f, 0.0f);
                    m_TargetCameraState.Translate(VecMovVal);//move cam up
                }
            }
